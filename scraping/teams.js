  ...
  for (const team of TEAMS) {
    ...
    for (const el of $lis) {
      ...
      if (roleLowerCase !== "presidente" && roleLowerCase !== "entrenador") {
        ...
        const playerStatsClass = $el
          .find(".id-player")
          .text()
          .replace(" ", "-");
        const $stats = $(
          `.${playerStatsClass} .data-player, .${playerStatsClass} .data-goalk`
        );
        ...
        if ($stats.length > 0) {
          ...
          $statsForPlayer.each((_, el) => {
            const $statEl = $(el);
            const statTitle = cleanText(
              $statEl.find(".el-meta").text()
            ).toLowerCase();
        ...
        const statsScore = availableStats
          ? Object.values(statsInfo).reduce((acc, value) => acc + value) /
            availableStats
          : null;
